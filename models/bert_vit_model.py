...
class Bert_VitClass(torch.nn.Module):
    def __init__(self, opt):
        ...
        if opt["label_type"] == "2_way":
            self.category_classifier = torch.nn.Linear(768+384+768, 2)  # Bert base 的H是768
        elif opt["label_type"] == "3_way":
            self.category_classifier = torch.nn.Linear(768+384+768, 3)  # Bert base 的H是768
        else:  # 6_way
            self.category_classifier = torch.nn.Linear(768+384+768, 6)  # Bert base 的H是768
    ...
    def forward(self, ids, mask, token_type_ids, pixel_values, emo_ids, emo_mask, labels):
        ...
        emo_embeds = last_hidden_states[:, 0, :]
        emo_embeds = self.emotion_layer(emo_embeds)

        combined_output = torch.cat((text_embeds, emo_embeds, img_embeds), dim=1) # 试一下不直接将emotion的embedding拼起来，直接让模型返回text_embed和emo_embed，计算L2loss，让原本的模型得到的text_embed能更好的提取情绪信息 (embedding distillation)
