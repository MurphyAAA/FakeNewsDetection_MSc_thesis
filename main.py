...
def main(opt):
    if opt["model"] == "bert":
        ...
        experiment.set_dataloader(train_loader, val_loader, test_loader)
        fileName = f'{opt["output_path"]}/checkpoint_{opt["model"]}_text_epoch_0_{opt["label_type"]}.pth'
        ...
        for epoch in range(start_epoch, opt['num_epochs']):
            ...
            experiment.save_clip_checkpoint(
                f'{opt["output_path"]}/checkpoint_{opt["model"]}_text_epoch_{epoch}_{opt["label_type"]}.pth', epoch)
            print(f"EPOCH:[{epoch}]  EXECUTION TIME: {epoch_time:.2f}s")
        print("validation")
