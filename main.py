...
def main(opt):
    if opt["model"] == "bert":
        ...
        experiment.set_dataloader(train_loader, val_loader, test_loader)
        fileName = f'{opt["output_path"]}/checkpoint_{opt["model"]}_epoch_1_{opt["label_type"]}_text.pth'
        ...
        for epoch in range(start_epoch, opt['num_epochs']):
            epoch_time = experiment.train(epoch)
            experiment.save_clip_checkpoint(f'{opt["output_path"]}/checkpoint_{opt["model"]}_epoch_{epoch}_{opt["label_type"]}_text.pth', epoch)
            print(f"EPOCH:[{epoch}]  EXECUTION TIME: {epoch_time:.2f}s")
        print("validation")
